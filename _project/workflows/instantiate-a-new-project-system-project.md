...
```json
{
  ...
  "steps": [
    ...
    { "id": "generate", "lane": "engines", "label": "Generate born-valid config", "detail": "init-config.mjs · proven to load via contract before write", "note": "init-config.mjs fills conventional defaults and proves the config loads through lib/contract.mjs before writing it — born valid, like a scaffolded entity.", "refs": [{ "rel": "references", "target": "decisions/0009-born-valid-config-generator" }], "to": ["wire"] },
