...
```json
{
  ...
  "steps": [
    ...
    { "id": "scaffold", "lane": "engines", "label": "Scaffold file", "detail": "new-entity.mjs · born valid", "note": "Derives the filename per the kind's scheme, writes valid frontmatter (status = the kind's initialStatus), lays down the section skeleton, and self-validates — conformant by construction.", "to": ["edit"] },
    ...
    { "id": "guard", "lane": "engines", "label": "Guard the save", "detail": "PreToolUse · blocks contract breaks", "note": "The hook re-runs the single validateEntity check on every _project/ write and blocks (exit 2) anything that would break the contract. Read-only, fails open.", "to": ["validate"] },
